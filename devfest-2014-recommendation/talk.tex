...
\begin{frame}
  ...
  \only<2>{
    \vspace{1cm}
    \gray{Et puis, partitionnement de données (\textit{clustering}),
      etc.}
  }
\end{frame}
...
\begin{frame}
  ...
  Mais,
  \begin{itemize}
  \item Exemple : 2 objets similaire / 2 utilisateurs similaires)
  \end{itemize}
...
\begin{frame}
  \frametitle{Partitionnement de données \textit{(clustering)}}

  \begin{itemize}
  \item kNN \only<2>{\gray{$k$-Nearest Neighbor}}
  \item Fléau de la dimension \only<2>{\gray{malédiction, \textit{curse}}}
