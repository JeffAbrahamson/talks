...
\begin{frame}
  ...
  \cnote {
    ...
    TODO: This needs supporting slides with computations.  We'll also
    have to come back to it when we understand about $p$ values and
    the like.
...
\begin{frame}
  ...
  \only<1-5> {
    ...
    \item Probability of getting 0\%? \only<3-5>{\blue{$\left(\frac 14\right)^{10}$}}
    \item Expected score? \only<4-5>{\blue{$\E(X) = 10\left(\frac 14\right) = \frac
        52$}}
    \item Standard deviation \only<5>{\blue{$\var(X) = \frac 14 \frac 34 = \frac 3{16}$}\only<6>{$10\left(\frac 3{16}\right) = \frac {15}{8}$}}
    \end{itemize}
    We'll come back to: if student gets 9 of 10 correct, do we really
    believe that they guessed at random?
  }

  \cnote{Anyone in the room worked in finance?}
  \only<6> {
