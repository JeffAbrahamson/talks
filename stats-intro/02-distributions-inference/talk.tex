...
\begin{frame}
  ...
  \only<2> {
    \begin{quote}
      ...
      x = np.linspace(ss.uniform.ppf(.01), \\
      \hspace{1cm}ss.uniform.ppf(.99), 100)\\
  ...
  \only<3>{
    \begin{quote}
      \tt
      \gray{x = np.linspace(ss.norm.ppf(.01), \\
...
\begin{frame}
  ...
  \only<3> {
    ...
    \note { The pmf$(k)$ (mass function) is $1-p$ if $k=0$ and $p$ if $k=1$. }
  }
...
\begin{frame}
  ...
  \only<4> {
    \begin{quote}
      \tt
      x = np.linspace(ss.geom.ppf(.01, p), \\
      \hspace{1cm}ss.geom.ppf(.99, p), 100)\\
  ...
  \only<5>{
    \begin{quote}
      \tt
      \gray{x = np.linspace(ss.geom.ppf(.01, p), \\
      \hspace{1cm}ss.geom.ppf(.99, p), 100) } \\
  ...
  \note{
    ...
    TODO: which of the two definitions is this???
  }
...
\begin{frame}
  ...
  \vspace{6mm}
  \only<1> { (Sometimes we say $\mu$ instead of $\lambda$.) }
  ...
  \only<4>{
    \begin{quote}
      \tt
      \gray{x = np.linspace(ss.poisson.ppf(.01, mu), \\
      \hspace{1cm}ss.poisson.ppf(.99, mu), 100) }\\
  ...
  \only<5>{
    ...
    \note{Sample random variable.  Exercise: histogram.}
  }
...
\begin{frame}
  ...
  \only<4>{
    \begin{quote}
      \tt
      \gray{x = np.linspace(ss.binom.ppf(.01, n, p), \\
      \hspace{1cm}ss.binom.ppf(.99, n, p), 100) }\\
...
\begin{frame}
  ...
  \note {
    ...
    TODO: I haven't introduced $Z$ value, put off to then?
  }
...
\begin{frame}
  ...
  \only<4>{
    \begin{quote}
      \tt
      \gray{x = np.linspace(ss.nbinom.ppf(.01, n, p), \\
      \hspace{1cm}ss.nbinom.ppf(.99, n, p), 100) }\\
...
\begin{frame}
  ...
  \note {
    TODO: Get data sets.  Show a/b test.
  }
\end{frame}
