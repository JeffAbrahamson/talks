...
\begin{frame}
  ...
  \only<3>{
    \begin{quote}
      \tt
      \gray{x = np.linspace(ss.uniform.ppf(.01), \\
      \hspace{1cm}ss.uniform.ppf(.99), 100)\\}
...
\begin{frame}
  ...
  \only<2> {
    ...
    \end{quote}
    \cnote{TODO: Define skew, kurtosis.}
  }
  \only<3> {
    \begin{quote}
      \tt
    ...
    \cnote { The pmf$(k)$ (mass function) is $1-p$ if $k=0$ and $p$ if $k=1$. }
  }
...
\begin{frame}
  ...
  \only<5>{
    ...
    \cnote{Sample random variable.  Exercise: histogram.}
  }
