...
\begin{frame}
  ...
  \only<3>{\cimghh{cluster-2.png}}
  \only<4>{\cimghh{cluster-3.png}}
  \only<5>{\cimgg{cluster-4.png}}
\end{frame}

\begin{frame}
  \frametitle{$k$ Nearest Neighbours}
  \only<1>{\cimgh{plot-similar-1.png}}
  \only<2>{\cimgh{plot-similar-2.png}}
  \only<3>{\cimgh{plot-similar-3.png}}
  \only<4>{\cimgh{plot-similar-4.png}}
  \only<5>{\centerline{\scalebox{.5}{\input{knn.pdf_t}}}}
\end{frame}
...
\begin{frame}[t]
  \frametitle{Support Vector Machine}
  \vspace{1cm}
\centerline{\scalebox{.6}{\input{not-linearly-separable.pdf_t}}}
\only<2>{
  \bigskip
  \begin{displaymath}
...
\begin{frame}
  ...
  \phrase{Yes, but it's a work in progress\dots}
\end{frame}

\begin{frame}
  \frametitle{There's more!}
  \only<1>{\phrase{I could capture the active window}}
  \only<2>{\cimgh{gtd-img-1.png}}
  \only<3>{\cimggg{gtd-img-2.png}}
  \only<4>{\cimggg{gtd-img-3.png}}
  \only<5>{\cimgh{gtd-img-4.png}}
\end{frame}
